...
import { QrCode, ArrowLeft, RefreshCw, Bus, Train, Shuffle } from "lucide-react";
  ...
  if (!latestTicket) {
    return (
      <div className="py-20 text-center space-y-4">
        ...
        <Link href="/">
          <Button variant="link" className="text-[#060267]">Search Routes</Button>
        </Link>
      </div>
    );
  }

  const Icon = latestTicket.type === "Combined" ? Shuffle : latestTicket.type === "Train" ? Train : Bus;

  return (
    <div className="space-y-6 animate-in zoom-in-95 duration-500 fade-in pb-10">
      ...
      >
        <Card className="text-center overflow-hidden border-0 shadow-2xl bg-white max-w-sm mx-auto relative group">
          <div className="absolute top-0 w-full h-3 bg-gradient-to-r from-[#060267] to-[#49a5d7]"></div>
          <CardContent className="pt-8 pb-6 px-6 relative">
            <div className="inline-flex items-center gap-2 bg-[#060267]/5 px-3 py-1 rounded-full text-[#060267] font-black text-[10px] uppercase tracking-tighter mb-2">
              <Icon className="w-3 h-3" /> {latestTicket.type} Pass
            </div>
            <h2 className="text-xl font-black text-[#060267] tracking-tighter uppercase">{latestTicket.route}</h2>
            
            <div className="my-8 flex justify-center relative">
              <div className="absolute inset-0 bg-[#92c01f]/5 rounded-xl blur-xl transition-all group-hover:bg-[#92c01f]/20"></div>
            ...
            <div className="space-y-3 mt-4 text-left border-t border-dashed border-gray-200 pt-5">
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-black text-red-600">{latestTicket.expiry}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
